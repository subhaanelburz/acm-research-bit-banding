...
int main(void)
{
    ...
    for(i = 0; i < 10000; i++)
    {
        // create a pointer that points to the bit-banded address of bit 0 so we can just ++; in loop
        volatile uint32_t* bb_address = (volatile uint32_t*) base_bb_address;

        for(j = 0; j < 32; j++)
        {
            // write to value of the bit-banded address
            *bb_address = (0x89ABCDEF >> j) & 0x1;

            // increment to the next bit (since its a ptr it will auto add the 4 from the bit-banding equation)
            bb_address++;
        }
    }
